...
void brute_force_nw(float icar_data[][ICAR_LATITUDE][ICAR_LONGITUDE], int dif_nw[][ICAR_TIME_DIM], int time_dim) {
    // set the indices for the icar data to only consider the north_west region
    int left_lon_icar = 0;
    int right_lon_icar = 80;
    
    int lower_lat_icar = 120;
    int upper_lat_icar = 200;
    ...
    for (cur_left_lon = 0; cur_left_lon < 80; cur_left_lon += 10) {
        for (cur_lower_lat = 120; cur_lower_lat < 200; cur_lower_lat += 10) {
                ...
                for (comp_day = 0; comp_day < time_dim; ++comp_day)  {
                    ...
                    for (lat = cur_lower_lat; lat < cur_upper_lat; lat++) {
                        for (lon = cur_left_lon; lon < cur_right_lon; lon++) {
                            ...
                            float dif = (icar_data[day][lat][lon] - icar_data[comp_day][lat][lon]) * (icar_data[day][lat][lon] - icar_data[comp_day][lat][lon]);
                            // add it to the sum
                            sum_difs += dif;
                        } 
                    }
...
int main(int argc, char** argv) {
    ...
    char* icar_dir_1980 = "/glade/p/ral/hap/trude/conus_icar/orig_and_removed_biasc_icar_data/merged_era/merged_era_hist_1980.nc";
    ...
    char* obs_dir_1980 = "/glade/p/ral/hap/common_data/Maurer_met_full/pr/nldas_met_update.obs.daily.pr.1980.nc";
    ...
    int dif_nw[64][icar_info.time_dim];
    
    // find the days with closest precipitation rate for each day and grid
    brute_force_nw(full_icar, dif_nw, icar_info.time_dim);
