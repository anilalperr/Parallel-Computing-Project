...
__global__ void find_difs(float* icar_data, int* dif_nw) {
    ...
    int cur_left_lon = (blockIdx.x / 8) * 10;
    int cur_lower_lat = (blockIdx.x % 8) * 10 + 120;
    ...
    int start_time = threadIdx.x * (ICAR_TIME_DIM/ THREADS_PER_BLOCK);
      
    int end_time;

    if (threadIdx.x != THREADS_PER_BLOCK-1) {
      end_time = start_time + ICAR_TIME_DIM/ THREADS_PER_BLOCK;
    }
    else {
      end_time = ICAR_TIME_DIM;
    }
    ...
    int comp_day;
    // iterate over time
    for (day = start_time; day < end_time; ++day){
        ...
        int* cur_pointer = dif_nw + (blockIdx.x * ICAR_TIME_DIM + day);
...
int main(int argc, char** argv) {
    ...
    find_difs<<<NUM_GRIDS, THREADS_PER_BLOCK>>>(p_icar_dev, p_dif_dev);
